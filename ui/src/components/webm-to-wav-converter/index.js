    ...
    for (index = 0; index < length; ++index) {
        for (channel = 0; channel < channels; ++channel) {
            ...
            if (bitDepth === 16) {
                ...
                console.log('DEBUG-EXTENSION::', 'Invalid bit depth for PCM encoding.');
                return;
            }
...
async function _getAudioBuffer(blobData, audioContext) {
 ...
 * @returns 
 */
async function getWaveBlob(blobData, as32BitFloat, audioContext, audioBuffer = null) {
    if (audioBuffer == null) {
        audioBuffer = await _getAudioBuffer(blobData, audioContext);
    }
    // Encoding setup.
 ...
 * @returns
 */
async function downloadWav(blobData, as32BitFloat) {
    const blob = await getWaveBlob(blobData, as32BitFloat);
...
module.exports = { getWaveBlob, downloadWav };